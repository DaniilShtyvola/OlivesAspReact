...
namespace OilNewsAPI.Database.Repositories
{
    public class NewsRepository : INewsRepository
    {
        ...
        {
            using var connection = new SqlConnection(GetConnectionString());
            var sql = "INSERT INTO News (Title, Content, Publisher, PublishDate) VALUES (@Title, @Content, @Publisher, @PublishDate)";
        ...
        {
            using var connection = new SqlConnection(GetConnectionString());
            var sql = "UPDATE News SET Title = @Title, Content = @Content, Publisher = @Publisher, PublishDate = @PublishDate WHERE Id = @Id";
            return await connection.ExecuteAsync(sql, news);
        }
